...
真实上游验证显示，单纯移除浏览器 CORS 仍不足以保证完整数据：东方财富指数历史连接会中断，全市场接口会出现声明总量与实际页数不一致。因此在不扩大本地路由白名单的前提下，新增腾讯指数日 K 和新浪全市场分页两条服务端备援；新浪响应按 GB18030 解码。为防止免费接口并发拥塞，本地模式采用 60 秒前端超时和数据域顺序刷新。最终真实链路覆盖率为 77%，仅前瞻 PE 按设计保持缺失。
