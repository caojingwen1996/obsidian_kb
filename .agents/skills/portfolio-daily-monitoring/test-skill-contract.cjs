...
assert.match(skill, /Markdown 母稿和可直接打开的 HTML 阅读版/);
assert.match(skill, /scripts\/render_report_html\.py/);
assert.match(skill, /每个标的第 6 节必须各自保留/);
assert.match(skill, /node scripts\/build\.mjs/);
assert.match(skill, /<六位代码>-<名称>-每日监控-YYYY-MM-DD/);
assert.match(skill, /601208-东材科技-每日监控-2026-08-06\.html/);
assert.match(skill, /持仓今日监控汇总-YYYY-MM-DD/);
assert.match(skill, /顶部“每日监控”按钮只允许链接最新汇总 HTML/);
assert.match(summaryTemplate, /## 三、逐标的监控索引/);
assert.match(summaryTemplate, /只列确实建议更新研报的标的/);
assert.doesNotMatch(summaryTemplate, /\| 标的 \| 是否建议更新 \| 估值处置状态 \|/);
assert.match(summaryTemplate, /\[打开报告\]\(六位代码-标的名称-每日监控-YYYY-MM-DD\.html\)/);
