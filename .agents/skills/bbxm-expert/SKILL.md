...
分析、比较、风险模型、观察记录与研报完成后，必须调用 [通用 HTML 报告输出](../bbxm-html-report/SKILL.md)，读取其工作流与样式规范。调用顺序为“分析子技能完成完整内容 → 通用HTML输出 → 格式、脚本与实际渲染检查 → 交付链接”；用户不需要额外逐个调用。

- HTML必须完整承载所选子技能模板，保留方法、证据、反证、缺口和状态；不能只生成摘要卡片。未完成的方法论环节先补齐。
- 业务模板规定内容、顺序、数据口径、输出位置及覆盖规则；通用技能统一展示样式和交付检查。已有更严格的离线或数据审计规则继续执行，不绕过。
- 已由子技能生成的HTML直接按通用规范核验，必要时原地修正获准更新的阅读版，不再生成第二套重复报告。保留业务Markdown/JSON工件。
- 风险模型和观察记录在 `tools/a-share-market-dashboard/data/Risk/` 原位置生成同名HTML；其他任务沿用自身输出位置。
- 简短问答、单数字查询仍用Markdown；用户明确仅聊天、不生成文件、纯JSON或指定其他格式时从其要求。纯机器输出不被HTML包装。
