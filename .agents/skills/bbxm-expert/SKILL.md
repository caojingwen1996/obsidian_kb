...
7. 用户是否要求直接分析体系三要素，判断竞争格局、流动性、情绪位置或三者共振，且主动作不是信息过滤、交易复盘、完整公司研究、个股产业思维或产业分析？是则使用 `bbxm-three-factor-analysis`。
8. 用户是否只要求判断风险增强、持平、减弱或重新增强，且不是在过滤单条信息、复盘交易、产出完整研报或分析产业路径？是则使用 `bbxm-risk-identification`。
9. 如果一句话同时满足多个条件，优先选择用户的主动作；信息过滤、交易复盘和显式估值请求分别优先于直接子技能路由。
