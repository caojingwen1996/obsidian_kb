...
- 可用中文调用：“使用 Archify，根据指定 Wiki 页面绘制风险传导路径图。” 默认使用中文内容与 `meta.locale: "zh-CN"`。
- 用有向边表达材料中有依据的传导机制；在节点说明中区分事实、观点、假设与待验证，不把图的可达性当成已经发生的风险或因果证明。主链、分支、反馈仅依据输入材料整理。
- 页面、来源与产物归属遵守项目 `AGENTS.md` 的 Wiki / Workbench 路由；安装示例不作为研究结论入库。
- 本地变更记录见 [技能变更日志](log.md)。修改本技能说明、模板、references、脚本或其他随附文件后，追加日期、版本变化、原因、涉及文件、具体变更与验证结果，并同步项目根 `log.md`。保留已有日志，不补写未经核实的历史。
