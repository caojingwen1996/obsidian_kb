...
- [ ] 第 2 步：确认目标日期和运行模式
- [ ] 第 2.5 步：实际抓取前识别抓取数量与风控风险，必要时自动降频
...
进入详情页实际抓取前，先完成候选发现并统计本轮抓取数量。数量预检只是风险估计，不保证平台一定触发或不触发风控，但可以避免用高频、固定节奏连续访问大量详情页。

- 默认使用 `--request-pacing auto`。
- 候选详情页少于 15 篇时，按现有正常节奏执行。
- 候选详情页达到 15 篇时，按高风险任务处理：详情页之间增加 4–8 秒随机间隔；每抓取 10 篇再增加 30–60 秒随机冷却。
- 操作者可用 `--request-pacing cautious` 主动启用谨慎节奏；只有明确判断当前环境安全时才使用 `normal` 跳过自动降频。
- 必须先记录抓取数量、风险等级和采用的节奏，再开始打开详情页。

完整流程见：[references/workflow.md](references/workflow.md#step-25-estimate-capture-volume-and-choose-pacing)
...
- 第 3 步必须复用第 2.5 步的抓取数量与节奏计划，不得在高风险任务中恢复为固定高频访问
