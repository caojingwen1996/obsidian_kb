...
- 接入来源：已安装的 strategy-backtest-expert 插件 `1.0.0+codex.20260605023011`；工具版本 `1.0.4`。完整程序及 references 已复制到本项目，不依赖插件缓存路径。
- 在项目根目录执行：`node .agents/skills/westock-data/scripts/index.js <命令> <参数>`。本文所有 `westock-data ...` 示例均使用此入口替换命令前缀；无需全局安装或 npm install。
- 示例：`node .agents/skills/westock-data/scripts/index.js quote sh600000`。
- 环境要求：Node.js >= 18；先用 `node --version` 和 `node .agents/skills/westock-data/scripts/index.js --help` 检查环境。
- 本工具作为可显式调用的补充数据源；其他研究技能已规定的数据源优先级仍按各技能执行。
- 引用结果时记录查询日期、数据日期、标的代码、币种及实际来源。空结果、错误或缺失字段应标为“待验证”，不得补造数据。
- 具体标的研究及行情快照按项目规则进入 `workbench/`；可复用知识满足回流门槛后才进入 `wiki/`。
- 维护日志：[log.md](./log.md)。修改本技能说明、参考文档或程序时，必须追加日期、版本变化（未升级则明确版本号不变）、原因、涉及文件、具体变更和验证结果，并同步根 `log.md`。
