...
2. `核心观点` 必须在通读该帖完整正文后提炼，优先写成“对象 + 判断”或“主题 + 结论”；禁止直接使用首句、首句截断、雪球页面自动标题或正文机械截取作为文件名。
3. 核心观点必须忠于原帖，不得补入帖子没有表达的结论；无法可靠提炼时，记录为未解决项，不得用首句回退。
4. 文件名必须清理 Windows 禁用字符 `<>:"/\|?*`、控制字符、尾部空格和尾部句点，同时保留可读的中文语义。
5. 同一作者在同一秒出现相同核心观点时，后续文件依次使用 `HHMMSS_核心观点_2.md`、`HHMMSS_核心观点_3.md`；对应解读文件必须使用相同序号，如 `HHMMSS_核心观点_2_解读.md`。
6. 帖子 URL 或稳定来源标识仍是去重主键；文件名只用于阅读和定位，不得替代 `state.json` 中的稳定键。

### 3.2 逐帖解读

每个已保存原帖都必须生成对应的 `_解读.md` 文件。解读文件只允许使用以下结构：
...
<基于对应帖子完整正文和本任务必读知识页形成的解读正文>
```

除标题外，正文只能保留“冰冰小美体系解读”这一个章节，不得增加“核心观点”“观点依据”“风险节点”“行为翻译”“相关页面”或其他独立章节。

解读要求：

1. 必须基于对应帖子完整正文和本提示词要求阅读的冰冰小美体系知识页，使用体系自身的概念和判断方式。
2. 不得把模型常识写成作者观点，不得联网补充行情、新闻、利率、资金流或其他外部数据。
3. 证据不足、体系映射不明确或原帖语义不完整时，必须在解读正文中明确标注“待验证”，不得强行解释。
4. `_解读.md` 只作为逐帖解读，不计入原始帖数量、`saved_post_count` 或风险节点次数。
