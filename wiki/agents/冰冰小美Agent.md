...
- **保存规则**：两个风险技能默认将完整模型、上游结果、逐轮记录与定位表保存到项目内 `tools/a-share-market-dashboard/data/Risk/`，具体目录结构遵守共享交接契约；用户明确仅对话或指定其他位置时优先遵从。旧 Workbench 研究不自动迁移；保存不代表已接入看板展示或启动调度。
